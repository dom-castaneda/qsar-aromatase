"""
CDK SubstructureFingerprinter -- native Python/RDKit implementation.

307 SMARTS patterns from SMARTS_InteLigand.txt by Christian Laggner,
Inte:Ligand Software-Entwicklungs und Consulting GmbH (LGPL).
Source: https://github.com/cdk/cdk  (descriptor/fingerprint/.../data/SMARTS_InteLigand.txt)

Usage:
    from substructure_fingerprint import compute_substruct_fp, PATTERN_NAMES
    from rdkit import Chem
    mol = Chem.MolFromSmiles("c1ccccc1")
    bits = compute_substruct_fp(mol)   # list of 307 ints (0 or 1)
"""
from rdkit import Chem

# ---------------------------------------------------------------------------
#  307 named SMARTS patterns (index = bit position)
# ---------------------------------------------------------------------------
PATTERNS = [
...
# Pattern names in bit order (for column headers, interpretability)
PATTERN_NAMES = [name for name, _ in PATTERNS]

# Pre-compile SMARTS into RDKit mol objects at import time.
# Patterns that fail to parse (e.g. disconnected "Salt" SMARTS) get None
# and their bit is always 0.
_COMPILED = []
for _name, _smarts in PATTERNS:
    _mol = Chem.MolFromSmarts(_smarts)
    _COMPILED.append(_mol)

N_BITS = len(PATTERNS)  # 307


def compute_substruct_fp(mol):
    """Return a list of 307 ints (0/1) for the given RDKit Mol object."""
    bits = []
