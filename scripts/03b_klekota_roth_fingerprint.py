"""
Klekota-Roth Fingerprinter -- native Python/RDKit implementation.

4860 SMARTS patterns from KlekotaRothFingerprinter.java in the CDK.
Based on: Klekota & Roth (2008) "Chemical substructures that enrich
for biological activity", Bioinformatics 24(21):2518-2525.
Source: https://github.com/cdk/cdk (public domain)

Usage:
    from klekota_roth_fingerprint import compute_kr_fp, N_BITS
    from rdkit import Chem
    mol = Chem.MolFromSmiles("c1ccccc1")
    bits = compute_kr_fp(mol)   # list of 4860 ints (0 or 1)
"""
from rdkit import Chem

# ---------------------------------------------------------------------------
#  4860 SMARTS patterns (index = bit position)
# ---------------------------------------------------------------------------
SMARTS = [
    '[!#1][CH]([!#1])[!#1]',
    '[!#1][CH]([!#1])[CH]([!#1])[!#1]',
...
N_BITS = len(SMARTS)  # 4860

# Pre-compile SMARTS into RDKit mol objects at import time.
_COMPILED = [Chem.MolFromSmarts(s) for s in SMARTS]


def compute_kr_fp(mol):
    """Return a list of 4860 ints (0/1) for the given RDKit Mol object."""
    bits = []
