...
# ===================================================================
with st.container(border=True):
    st.subheader("Training Time Heatmap (seconds)")
    st.caption("Shows computational cost for each model-fingerprint combination. "
               "Useful for identifying efficiency trade-offs — some models achieve similar accuracy but train much faster.")

    time_sort = st.radio("Ordering", ["Alphabetical", "By time"], horizontal=True, key="time_sort")

    col1, col2 = st.columns(2)
    for i, split_name in enumerate(["Random", "Kennard-Stone"]):
        sub = df[df["Split"] == split_name]
        pivot_time = sub.pivot_table(index="Model", columns="Fingerprint", values="Time_s")
        if time_sort == "Alphabetical":
            pivot_time = pivot_time.sort_index(axis=0).sort_index(axis=1)
        else:
            pivot_time = pivot_time.loc[pivot_time.mean(axis=1).sort_values(ascending=False).index]
            pivot_time = pivot_time[pivot_time.mean(axis=0).sort_values(ascending=False).index]

        fig_time = px.imshow(pivot_time, text_auto=".1f", color_continuous_scale="YlOrRd",
                             aspect="auto", title=f"{split_name} — Time (s)")
...
with st.container(border=True):
    ...
    st.caption("Each point represents a model+fingerprint combination. Points above the diagonal indicate "
               "Kennard-Stone outperforms Random for that combination; below means Random is better.")
...
with st.container(border=True):
    ...
    st.caption("Shows the top-performing model for each fingerprint type, helping identify which algorithm "
               "works best with each molecular representation.")
...
with st.container(border=True):
    ...
    st.caption("Average performance of each model across all fingerprints. "
               "Reveals which algorithms are consistently strong regardless of molecular representation.")
...
with st.container(border=True):
    ...
    st.caption("Full results for all 384 model runs (16 models x 12 fingerprints x 2 splits). "
               "Includes number of descriptors (features after near-constant removal) for each fingerprint.")

    # Add number of descriptors per fingerprint
    FP_DESCRIPTOR_COUNT = {
        "MACCS": 114,
        "PubChem": 309,
        "Substructure": 41,
        "SubstructureCount": 43,
        "KR": 172,
        "KR_Count": 173,
        "AtomPairs2D": 518,
        "AP2D_Count": 455,
